//+------------------------------------------------------------------+
//|  ChartAnalyzer_M1.mq5                                            |
//|  Captures OHLC + screenshot on candle close → Python API        |
//|  Configured for M1 (1-minute) timeframe                         |
//+------------------------------------------------------------------+
...
input ENUM_TIMEFRAMES  Timeframe_     = PERIOD_M1;                         // Timeframe (M1)
input int              OHLCBars       = 100;                               // Number of bars to send
...
int OnInit()
{
   Log("ChartAnalyzer_M1 EA initialising. Server=" + ServerURL);
   g_ScreenshotFile = TerminalInfoString(TERMINAL_DATA_PATH) +
                      "\\MQL5\\Files\\chart_snapshot_m1.png";
   ...
   Log("Timer set to 1s (M1 candle sync). EA ready. Current candle: " + TimeToString(g_LastCandleTime));
...
{
   EventKillTimer();
   Log("ChartAnalyzer_M1 EA stopped. Reason=" + IntegerToString(reason));
}

//+------------------------------------------------------------------+
//| Timer event – chạy mỗi 1 giây, detect nến M1 mới đóng          |
//+------------------------------------------------------------------+
void OnTimer()
{
   ...
   Log("=== Nến M1 mới: " + TimeToString(currentCandleTime) +
...
bool CaptureChart()
{
   ...
   string relFile = "chart_snapshot_m1.png";
...
//+------------------------------------------------------------------+
//| Build JSON array of last N OHLC bars (M1 timeframe)             |
//+------------------------------------------------------------------+
string GetOHLC()
{
   MqlRates rates[];
...
string EncodeBase64(const string filePath)
{
   ...
   string shortName = "chart_snapshot_m1.png";
...
void SendRequest(const string ohlcJson, const string b64Image)
{
   ...
   string timeframe_val = EnumToString(Timeframe_);  // "PERIOD_M1"
   // Trim "PERIOD_" prefix cho gọn: M1
